...
export const PermitForm: React.FC<PermitFormProps> = ({ initialData, onSubmit, onCancel, userRole = 'Super Admin' }) => {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">
            {isAuditor ? 'Registry View' : initialData ? 'Update Record' : 'Issue Permit'}
          </h1>
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-widest mt-1">
            {isAuditor ? 'Municipal Registry Lookup' : 'Registry Entry Management'}
          </p>
        </div>
        <button onClick={onCancel} className="px-4 py-2 text-sm font-bold text-slate-400 hover:text-slate-900 transition-colors">
          {isAuditor ? 'Close View' : 'Discard Changes'}
        </button>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        {/* Design Settings Section - Only for Admins */}
        {!isAuditor && (
        ...
        <div className={`bg-white p-8 rounded-3xl border border-slate-200 shadow-sm space-y-6 ${isAuditor ? 'opacity-75 grayscale-[0.3]' : ''}`}>
          <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-1.5">
              ...
              <input 
                type="text" required disabled={isAuditor}
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all font-black text-lg tracking-tight uppercase disabled:cursor-not-allowed"
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" required disabled={isAuditor}
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all font-bold disabled:cursor-not-allowed"
            ...
            <div className="space-y-1.5">
              ...
              <input 
                type="text" required disabled={isAuditor}
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all disabled:cursor-not-allowed"
                value={formData.make}
            ...
            <div className="space-y-1.5 md:col-span-2">
              ...
              <select disabled={isAuditor}
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-emerald-500 outline-none appearance-none cursor-pointer font-bold disabled:cursor-not-allowed"
        ...
        <div className="flex gap-4 pt-6">
          ...
          >
            {isAuditor ? 'Close View' : 'Cancel'}
          </button>
          {!isAuditor && (
            <button 
              type="submit"
              className="flex-[2] px-8 py-4 text-white bg-emerald-600 shadow-emerald-600/20 hover:bg-emerald-700 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl transition-all active:scale-95"
            >
